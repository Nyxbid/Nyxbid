  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        {agents.map((a) => {
          const pct = a.daily_budget > 0 ? Math.round((a.spent_today / a.daily_budget) * 100) : 0;
            ...
            <div key={a.id} className="rounded-lg border border-border bg-card px-5 py-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-xs capitalize text-muted">{a.role}</p>
                </div>
              ...
              <div className="mt-4">
                <div className="flex items-baseline justify-between text-xs text-muted">
                  <span>{formatUsdc(a.spent_today)} / {formatUsdc(a.daily_budget)}</span>
                  <span>{pct}%</span>
                </div>
